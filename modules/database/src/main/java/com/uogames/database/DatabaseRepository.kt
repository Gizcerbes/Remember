...
class DatabaseRepository private constructor(private val database: MyDatabase) {
	...
	fun getCardFLOW(number: Int, like: String) =
		database.cardDAO().getCardFLOW(number, like).map { it?.toDTO() }

	suspend fun getRandomCard() = database.cardDAO().getRandom()?.toDTO()

	suspend fun getRandomCardWithout(phrase: String, translate: String) =
		database.cardDAO().getRandomWithout(phrase, translate)?.toDTO()
