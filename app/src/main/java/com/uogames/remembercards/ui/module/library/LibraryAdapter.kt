...
class LibraryAdapter(
				...
				model.getShareAction(mm.module).observeLaunching(this) {
					runCatching {
						...
						bind.btnShare.visibility =
							if (!it && isAvailableToShare(mm.module, model.isChanged(mm.module).value == true)) View.VISIBLE else View.GONE
	...
	inner class GlobalModuleHolder(private val bind: CardModuleBinding) : ClosableHolder(bind.root) {
		...
		private val endAction: (String?) -> Unit = {
			...
			it?.let { Toast.makeText(itemView.context, it, Toast.LENGTH_SHORT).show() }
		...
		override fun show() {
			...
			observer = recyclerScope.launch {
				...
				model.getDownloadFlow(mm.module.globalId).observe(this) {
					launch {
						if (it) startAction()
						else {
							endAction(null)
